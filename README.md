...
首选设置方式：运行入职向导 (`moltbot onboard`)。它会引导您完成网关、工作区、渠道和技能的配置。CLI 向导是推荐路径，适用于 **macOS, Linux, 和 Windows (通过 WSL2; 强烈推荐)**。
支持使用 npm, pnpm, 或 bun。
...
模型说明：虽然支持任何模型，但我强烈推荐 **Anthropic Pro/Max (100/200) + Opus 4.5**，因为它具有长上下文优势和更好的抗提示注入能力。参见 [入职指南](https://docs.molt.bot/start/onboarding)。

## 模型 (选择 + 认证)

- 模型配置 + CLI: [Models](https://docs.molt.bot/concepts/models)
- 认证配置文件轮换 (OAuth vs API keys) + 故障转移: [Model failover](https://docs.molt.bot/concepts/model-failover)
...
遗留说明：`clawdbot` 仍作为兼容性垫片 (shim) 提供。
...
切换通道 (git + npm): `moltbot update --channel stable|beta|dev`。
详情：[开发通道](https://docs.molt.bot/install/development-channels)。
...
注意：`pnpm moltbot ...` 直接运行 TypeScript (通过 `tsx`)。`pnpm build` 生成 `dist/` 用于通过 Node / 打包好的 `moltbot` 二进制文件运行。

## 安全默认设置 (私信访问)

Moltbot 连接到某些真实的消息平台。请将入站私信 (DM) 视为 **不可信输入**。
...
- 批准方式：`moltbot pairing approve <channel> <code>` (然后发送者会被添加到本地允许列表存储中)。
- 公共入站私信需要显式选择加入：设置 `dmPolicy="open"` 并在渠道允许列表 (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`) 中包含 `"*"`。

运行 `moltbot doctor` 来发现有风险/配置错误的私信策略。

## 亮点

- **[本地优先网关](https://docs.molt.bot/gateway)** — 会话、渠道、工具和事件的统一控制平面。
...
- **[多 Agent 路由](https://docs.molt.bot/gateway/configuration)** — 将入站渠道/账户/对等端路由到隔离的 Agent（工作区 + 每 Agent 会话）。
- **[语音唤醒 (Voice Wake)](https://docs.molt.bot/nodes/voicewake) + [交谈模式 (Talk Mode)](https://docs.molt.bot/nodes/talk)** — macOS/iOS/Android 上的始终在线语音，支持 ElevenLabs。
- **[Live Canvas](https://docs.molt.bot/platforms/mac/canvas)** — Agent 驱动的可视化工作区，支持 [A2UI](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui)。
- **[一流工具支持](https://docs.molt.bot/tools)** — 浏览器, Canvas, Nodes, Cron, Sessions 以及 Discord/Slack 动作。
- **[配套应用](https://docs.molt.bot/platforms/macos)** — macOS 菜单栏应用 + iOS/Android [Nodes](https://docs.molt.bot/nodes)。
- **[入职向导](https://docs.molt.bot/start/wizard) + [技能](https://docs.molt.bot/tools/skills)** — 向导式设置，支持捆绑/托管/工作区技能。
...
### 核心平台
- [网关 WS 控制平面](https://docs.molt.bot/gateway) 含会话、在线状态、配置、Cron、Webhooks、[控制 UI](https://docs.molt.bot/web) 和 [Canvas 主机](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui)。
...
- [Pi agent 运行时](https://docs.molt.bot/concepts/agent) RPC 模式，支持工具流式传输和块流式传输。
...
- [群组路由](https://docs.molt.bot/concepts/group-messages): 提及门控，回复标签，按渠道分块和路由。渠道规则：[Channels](https://docs.molt.bot/channels)。

### 应用 + Nodes
- [macOS 应用](https://docs.molt.bot/platforms/macos): 菜单栏控制平面，[Voice Wake](https://docs.molt.bot/nodes/voicewake)/PTT, [Talk Mode](https://docs.molt.bot/nodes/talk) 覆盖层, [WebChat](https://docs.molt.bot/web/webchat), 调试工具, [远程网关](https://docs.molt.bot/gateway/remote) 控制。
- [iOS Node](https://docs.molt.bot/platforms/ios): [Canvas](https://docs.molt.bot/platforms/mac/canvas), [Voice Wake](https://docs.molt.bot/nodes/voicewake), [Talk Mode](https://docs.molt.bot/nodes/talk), 摄像头, 屏幕录制, Bonjour 配对。
- [Android Node](https://docs.molt.bot/platforms/android): [Canvas](https://docs.molt.bot/platforms/mac/canvas), [Talk Mode](https://docs.molt.bot/nodes/talk), 摄像头, 屏幕录制, 可选 SMS。
...
- [Canvas](https://docs.molt.bot/platforms/mac/canvas): [A2UI](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui) 推送/重置, eval, 快照。
- [Nodes](https://docs.molt.bot/nodes): 摄像头快照/剪辑, 屏幕录制, [location.get](https://docs.molt.bot/nodes/location-command), 通知。
...
- [在线状态](https://docs.molt.bot/concepts/presence), [正在输入指示器](https://docs.molt.bot/concepts/typing-indicators), 和 [用量跟踪](https://docs.molt.bot/concepts/usage-tracking)。
...
- [Nix 模式](https://docs.molt.bot/install/nix) 声明式配置; 基于 [Docker](https://docs.molt.bot/install/docker) 的安装。
...
- **[网关 WebSocket 网络](https://docs.molt.bot/concepts/architecture)** — 客户端、工具和事件（以及运维）的单一 WS 控制平面：[Gateway runbook](https://docs.molt.bot/gateway)。
...
- **[浏览器控制](https://docs.molt.bot/tools/browser)** — moltbot 管理的 Chrome/Chromium 带 CDP 控制。
- **[Canvas + A2UI](https://docs.molt.bot/platforms/mac/canvas)** — Agent 驱动的可视化工作区 (A2UI 主机: [Canvas/A2UI](https://docs.molt.bot/platforms/mac/canvas#canvas-a2ui))。
- **[语音唤醒](https://docs.molt.bot/nodes/voicewake) + [交谈模式](https://docs.molt.bot/nodes/talk)** — 始终在线的语音和连续对话。
- **[Nodes](https://docs.molt.bot/nodes)** — Canvas, 摄像头快照/剪辑, 屏幕录制, `location.get`, 通知, 加上 macOS 专用的 `system.run`/`system.notify`。

## Tailscale 访问 (网关仪表板)

Moltbot 可以自动配置 Tailscale **Serve** (仅 tailnet) 或 **Funnel** (公开)，同时网关保持绑定到 loopback。配置 `gateway.tailscale.mode`:
...
详情：[Nodes](https://docs.molt.bot/nodes) · [macOS app](https://docs.molt.bot/platforms/macos) · [网关协议](https://docs.molt.bot/concepts/architecture)
...
- [从文档索引开始导航和了解“什么在哪”。](https://docs.molt.bot)
- [阅读架构概览以了解网关 + 协议模型。](https://docs.molt.bot/concepts/architecture)
- [当您需要每个键和示例时使用完整的配置参考。](https://docs.molt.bot/gateway/configuration)
- [按照操作手册运行网关。](https://docs.molt.bot/gateway)
- [了解控制 UI/Web 界面如何工作以及如何安全地暴露它们。](https://docs.molt.bot/web)
- [通过 SSH 隧道或 tailnets 了解远程访问。](https://docs.molt.bot/gateway/remote)
- [跟随入职向导流程进行引导式设置。](https://docs.molt.bot/start/wizard)
...
Moltbot 是为 **Molty** 打造的，一只太空龙虾 AI 助手。 🦞
